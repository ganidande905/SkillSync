...
export async function submitInterests(
  ...
  try {
    ...
    return { success: false, message: "Failed to save interests" };
  }
}
...
export async function submitSkills(
  ...
  try {
    ...
    const data = getOnboardingData();
    const skills =
      skillsOverride ?? (data.skills as SelectedSkill[] | undefined) ?? [];
    ...
    return { success: false, message: "Failed to save skills" };
  }
}
...
export async function submitPastProjects(
  ...
  try {
    ...
    const projects =
      projectsOverride ??
      ((data.projects as unknown as PastProject[]) || []);

    if (!projects.length)
      return { success: true, message: "No projects to save" };


    const map = new Map<string, PastProject>();
    for (const p of projects) {
      if (p?.title) map.set(p.title, p);
    }
    const uniqueProjects = [...map.values()];
    ...
    return { success: false, message: "Failed to save past projects" };
  }
}